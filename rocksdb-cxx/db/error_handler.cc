...
std::map<std::tuple<BackgroundErrorReason, rocksdb_rs::status::Code,
                    rocksdb_rs::status::SubCode, bool>,
         rocksdb_rs::status::Severity>
    ErrorSeverityMap = {
        // Errors during BG compaction
        {std::make_tuple(BackgroundErrorReason::kCompaction,
                         rocksdb_rs::status::Code::kIOError,
...
STATIC_AVOID_DESTRUCTION(const rocksdb_rs::status::Status,
                         kOkStatus){rocksdb_rs::status::Status_OK()};
...
const rocksdb_rs::status::Status &ErrorHandler::HandleKnownErrors(
  ...
  if (new_bg_err.severity() >= rocksdb_rs::status::Severity::kFatalError &&
      auto_recovery) {
    auto_recovery = false;
  }
  ...
  if (!new_bg_err.ok()) {
    rocksdb_rs::status::Status s = new_bg_err.Clone();
...
const rocksdb_rs::status::Status &ErrorHandler::SetBGError(
    const rocksdb_rs::status::Status &bg_status, BackgroundErrorReason reason) {
  db_mutex_->AssertHeld();
  rocksdb_rs::status::Status tmp_status = bg_status.Clone();
  rocksdb_rs::io_status::IOStatus bg_io_err =
      rocksdb_rs::io_status::IOStatus_new(std::move(tmp_status));
  ...
  if (bg_io_err.GetScope() !=
    ...
    rocksdb_rs::status::Status bg_err = rocksdb_rs::status::Status_new(
        new_bg_io_err, rocksdb_rs::status::Severity::kUnrecoverableError);
    ...
    if (BackgroundErrorReason::kCompaction == reason) {
      ...
      rocksdb_rs::status::Status bg_err = rocksdb_rs::status::Status_new(
          new_bg_io_err, rocksdb_rs::status::Severity::kSoftError);
      CheckAndSetRecoveryAndBGError(bg_err);
      ...
      rocksdb_rs::status::Status bg_err = rocksdb_rs::status::Status_new(
          new_bg_io_err, rocksdb_rs::status::Severity::kHardError);
      CheckAndSetRecoveryAndBGError(bg_err);
      recover_context_ = context;
      return StartRecoverFromRetryableBGIOError(bg_io_err);
    }
...
rocksdb_rs::status::Status ErrorHandler::OverrideNoSpaceError(
  ...
  if (db_options_.allow_2pc &&
    ...
    *auto_recovery = false;
    return rocksdb_rs::status::Status_new(
        bg_error, rocksdb_rs::status::Severity::kFatalError);
  }
  {
    uint64_t free_space;
    if (db_options_.env->GetFreeSpace(db_options_.db_paths[0].path, &free_space)
            .eq(rocksdb_rs::status::Status_NotSupported())) {
      *auto_recovery = false;
    }
  }
...
rocksdb_rs::status::Status ErrorHandler::ClearBGError() {
  db_mutex_->AssertHeld();
...
rocksdb_rs::status::Status ErrorHandler::RecoverFromBGError(bool is_manual) {
...
ErrorHandler::StartRecoverFromRetryableBGIOError(
...
void ErrorHandler::RecoverFromRetryableBGIOError() {
  ...
  if (end_recovery_) {
    EventHelpers::NotifyOnErrorRecoveryEnd(
        ...
        rocksdb_rs::status::Status_ShutdownInProgress(), db_mutex_);
    return;
  }
  ...
  EventHelpers::NotifyOnErrorRecoveryEnd(
      db_options_.listeners, bg_error_,
      rocksdb_rs::status::Status_Aborted("Exceeded resume retry count"),
      db_mutex_);
...
void ErrorHandler::CheckAndSetRecoveryAndBGError(
    const rocksdb_rs::status::Status &bg_err) {
  if (recovery_in_prog_ && recovery_error_.ok()) {
