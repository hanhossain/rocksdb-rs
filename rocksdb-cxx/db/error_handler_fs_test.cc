  ...
  void OnTableFileCreationStarted(
    ...
    if (file_count_ > 0) {
      if (--file_count_ == 0) {
        ...
        file_creation_error_ = IOStatus_OK();
      }
    }
    cv_.SignalAll();
  }
...
TEST_F(DBErrorHandlingFSTest, FLushWriteError) {
  ...
  SyncPoint::GetInstance()->SetCallBack("FlushJob::Start", [&](void*) {
    fault_fs_->SetFilesystemActive(false, IOStatus_NoSpace("Out of space"));
  });
...
TEST_F(DBErrorHandlingFSTest, FLushWriteNoSpaceError) {
  ...
  rocksdb_rs::io_status::IOStatus error_msg = IOStatus_NoSpace("Retryable IO Error");
...
TEST_F(DBErrorHandlingFSTest, FLushWriteFileScopeError) {
  ...
  rocksdb_rs::io_status::IOStatus error_msg = IOStatus_IOError("File Scope Data Loss Error");
...
TEST_F(DBErrorHandlingFSTest, CorruptionError) {
  ...
  rocksdb::SyncPoint::GetInstance()->SetCallBack(
      "BackgroundCallCompaction:0", [&](void*) {
        fault_fs_->SetFilesystemActive(false,
                                       IOStatus_Corruption("Corruption"));
      });
  rocksdb::SyncPoint::GetInstance()->EnableProcessing();
...
TEST_F(DBErrorHandlingFSTest, FlushReadError) {
  ...
  SyncPoint::GetInstance()->SetCallBack(
      "BuildTable:BeforeOutputValidation", [&](void*) {
        rocksdb_rs::io_status::IOStatus st = IOStatus_IOError();
  ...
  SyncPoint::GetInstance()->SetCallBack(
      ...
      [&](void*) { fault_fs_->SetFilesystemActive(true, IOStatus_OK()); });
  SyncPoint::GetInstance()->EnableProcessing();
  s = Flush();
  ASSERT_EQ(s.severity(), rocksdb_rs::status::Severity::kSoftError);
  SyncPoint::GetInstance()->DisableProcessing();
...
TEST_F(DBErrorHandlingFSTest, AtomicFlushNoSpaceError) {
  ...
  SyncPoint::GetInstance()->SetCallBack("BuildTable:create_file", [&](void*) {
    rocksdb_rs::io_status::IOStatus st = IOStatus_NoSpace();
    fault_fs_->SetFilesystemActive(false, st);
  });
  SyncPoint::GetInstance()->SetCallBack(
      "BuildTable:BeforeDeleteFile",
...
TEST_P(DBErrorHandlingFencingTest, FLushWriteFenced) {
  ...
  ASSERT_OK(Put(Key(0), "val"));
  SyncPoint::GetInstance()->SetCallBack("FlushJob::Start", [&](void*) {
    fault_fs_->SetFilesystemActive(false, IOStatus_IOFenced("IO fenced"));
  });
  SyncPoint::GetInstance()->EnableProcessing();
  s = Flush();
