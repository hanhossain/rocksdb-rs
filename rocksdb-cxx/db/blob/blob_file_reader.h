...
class BlobFileReader {
 ...
 private:
  ...
  static rocksdb_rs::status::Status OpenFile(
  ...
  static rocksdb_rs::status::Status ReadFromFile(
      const RandomAccessFileReader* file_reader,
      const ReadOptions& read_options, uint64_t read_offset, size_t read_size,
      Statistics* statistics, Slice* slice, Buffer* buf,
      AlignedBuf* aligned_buf, Env::IOPriority rate_limiter_priority);

  static rocksdb_rs::status::Status VerifyBlob(const Slice& record_slice,
                                               const Slice& user_key,
                                               uint64_t value_size);

  static rocksdb_rs::status::Status UncompressBlobIfNeeded(
