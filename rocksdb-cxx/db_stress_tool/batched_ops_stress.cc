...
class BatchedOpsStressTest : public StressTest {
 public:
  ...
  rust::Vec<rocksdb_rs::status::Status> TestMultiGet(
    ...
    rust::Vec<rocksdb_rs::status::Status> ret_status = Status_new().create_vec(num_keys);
    ...
    for (size_t rand_key = 0; rand_key < num_keys; ++rand_key) {
      ...
      rust::Vec<rocksdb_rs::status::Status> statuses = Status_new().create_vec(num_prefixes);
      ReadOptions readoptionscopy = readoptions;
      readoptionscopy.snapshot = db_->GetSnapshot();
