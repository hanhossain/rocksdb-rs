...
ColumnFamilyOptions* ColumnFamilyOptions::OptimizeLevelStyleCompaction(
  ...
  for (int i = 0; i < num_levels; ++i) {
    if (i < 2) {
      compression_per_level[i] = rocksdb_rs::compression_type::CompressionType::kNoCompression;
    } else {
      compression_per_level[i] =
          LZ4_Supported()
              ? rocksdb_rs::compression_type::CompressionType::kLZ4Compression
              : (Snappy_Supported() ? rocksdb_rs::compression_type::CompressionType::kSnappyCompression : rocksdb_rs::compression_type::CompressionType::kNoCompression);
    }
  }
  return this;
}
