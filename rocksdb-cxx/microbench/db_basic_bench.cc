...
static void SetupDB(benchmark::State& state, Options& options,
  ...
  std::string db_path;
  rocksdb_rs::status::Status s = env->GetTestDirectory(&db_path);
  if (!s.ok()) {
...
static void TeardownDB(benchmark::State& state, const std::unique_ptr<DB>& db,
  ...
  uint64_t size;
  rocksdb_rs::status::Status s = db->GetApproximateSizes(&r, 1, &size);
  if (!s.ok()) {
    state.SkipWithError(s.ToString().c_str());
  }
...
static void DBOpen(benchmark::State& state) {
  ...
  for (auto _ : state) {
    {
      DB* db_ptr = nullptr;
      rocksdb_rs::status::Status s = DB::Open(options, db_name, &db_ptr);
      if (!s.ok()) {
        state.SkipWithError(s.ToString().c_str());
      }
      db.reset(db_ptr);
...
static void DBPut(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    rocksdb_rs::status::Status s = db->Put(wo, key, val);
    if (!s.ok()) {
      state.SkipWithError(s.ToString().c_str());
    }
  }
...
static void ManualCompaction(benchmark::State& state) {
  ...
  FlushOptions fo;
  rocksdb_rs::status::Status s = db->Flush(fo);
  if (!s.ok()) {
    state.SkipWithError(s.ToString().c_str());
  }
...
static void DBGet(benchmark::State& state) {
  ...
  if (negative_query) {
    for (auto _ : state) {
      std::string val;
      rocksdb_rs::status::Status s = db->Get(ro, kg_rnd.NextNonExist(), &val);
      if (s.IsNotFound()) {
        not_found++;
      }
    }
    ...
    for (auto _ : state) {
      std::string val;
      rocksdb_rs::status::Status s = db->Get(ro, kg_rnd.Next(), &val);
      if (s.IsNotFound()) {
        not_found++;
      }
    }
...
static void SimpleGetWithPerfContext(benchmark::State& state) {
  ...
  for (auto _ : state) {
    std::string val;
    get_perf_context()->Reset();
    rocksdb_rs::status::Status s = db->Get(ro, kg.NextNonExist(), &val);
    if (s.IsNotFound()) {
      not_found++;
    }
...
static void DBGetMergeOperandsInMemtable(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    rocksdb_rs::status::Status s = db->GetMergeOperands(
