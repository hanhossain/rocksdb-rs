...
class ReadOnlyFileSystem : public FileSystemWrapper {
  static inline rocksdb_rs::io_status::IOStatus FailReadOnly() {
    rocksdb_rs::io_status::IOStatus s = rocksdb_rs::io_status::IOStatus_IOError("Attempted write to ReadOnlyFileSystem");
 ...
 public:
  ...
  rocksdb_rs::io_status::IOStatus NewDirectory(const std::string& /*dir*/,
  ...
  rocksdb_rs::io_status::IOStatus CreateDirIfMissing(const std::string& dirname,
    ...
    bool is_dir = false;
    rocksdb_rs::io_status::IOStatus s = IsDirectory(dirname, options, &is_dir, dbg);
    if (s.ok() && is_dir) {
      return s;
    } else {
      return FailReadOnly();
  ...
  rocksdb_rs::io_status::IOStatus RenameFile(const std::string& /*src*/, const std::string& /*dest*/,
                      const IOOptions& /*options*/,
                      IODebugContext* /*dbg*/) override {
    return FailReadOnly();
  }
  rocksdb_rs::io_status::IOStatus LinkFile(const std::string& /*src*/, const std::string& /*dest*/,
                    const IOOptions& /*options*/,
                    IODebugContext* /*dbg*/) override {
    return FailReadOnly();
  }
