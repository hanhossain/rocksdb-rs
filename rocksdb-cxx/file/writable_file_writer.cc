...
namespace rocksdb {
rocksdb_rs::io_status::IOStatus WritableFileWriter::Create(
  ...
  rocksdb_rs::io_status::IOStatus io_s =
      fs->NewWritableFile(fname, file_opts, &file, dbg);
  if (io_s.ok()) {
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::Append(
    const Slice& data, uint32_t crc32c_checksum,
  ...
  if (perform_data_verification_ && buffered_data_with_checksum_ &&
    ...
    if (use_direct_io() || (buf_.Capacity() - buf_.CurrentSize()) >= left) {
      if ((buf_.Capacity() - buf_.CurrentSize()) >= left) {
        ...
        if (appended != left) {
          s = rocksdb_rs::io_status::IOStatus_Corruption(
              "Write buffer append failure");
        }
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::Pad(
    const size_t pad_bytes, Env::IOPriority op_rate_limiter_priority) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
  ...
  while (left) {
    ...
    left -= append_bytes;
    if (left > 0) {
      rocksdb_rs::io_status::IOStatus s = Flush(op_rate_limiter_priority);
      if (!s.ok()) {
        set_seen_error();
        return s;
      }
...
// enabled
rocksdb_rs::io_status::IOStatus WritableFileWriter::Flush(
    Env::IOPriority op_rate_limiter_priority) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::Sync(bool use_fsync) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }

  rocksdb_rs::io_status::IOStatus s = Flush();
  if (!s.ok()) {
    set_seen_error();
    return s;
  }
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::SyncWithoutFlush(
    bool use_fsync) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
  ...
  rocksdb_rs::io_status::IOStatus s = SyncInternal(use_fsync);
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::SyncInternal(
    bool use_fsync) {
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::RangeSync(uint64_t offset,
                                                              uint64_t nbytes) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
  ...
  rocksdb_rs::io_status::IOStatus s =
      writable_file_->RangeSync(offset, nbytes, io_options, nullptr);
  if (!s.ok()) {
    set_seen_error();
  }
  if (ShouldNotifyListeners()) {
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::WriteBuffered(
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::WriteBufferedWithChecksum(
    const char* data, size_t size, Env::IOPriority op_rate_limiter_priority) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
...

  {
    ...
    }
    {
      ...
      rocksdb_rs::coding_lean::EncodeFixed32(checksum_buf,
                                             buffered_data_crc32c_checksum_);
      v_info.checksum = Slice(checksum_buf, sizeof(uint32_t));
...
// offsets.
rocksdb_rs::io_status::IOStatus WritableFileWriter::WriteDirect(
    Env::IOPriority op_rate_limiter_priority) {
  if (seen_error()) {
    assert(false);
...
rocksdb_rs::io_status::IOStatus WritableFileWriter::WriteDirectWithChecksum(
    Env::IOPriority op_rate_limiter_priority) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
