...
uint64_t Hash64(const char* data, size_t n, uint64_t seed) {
  return XXPH3_64bits_withSeed(rust::Slice(reinterpret_cast<const uint8_t*>(data), n), seed);
}

uint64_t Hash64(const char* data, size_t n) {
  // Same as seed = 0
  return XXPH3_64bits(rust::Slice(reinterpret_cast<const uint8_t*>(data), n));
}
