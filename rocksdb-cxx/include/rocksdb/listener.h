...
struct BlobFileCreationBriefInfo : public FileCreationBriefInfo {
  BlobFileCreationBriefInfo(const std::string &_db_name,
                            const std::string &_cf_name,
                            const std::string &_file_path, int _job_id,
                            rocksdb_rs::types::BlobFileCreationReason _reason)
      : FileCreationBriefInfo(_db_name, _cf_name, _file_path, _job_id),
        reason(_reason) {}

  // reason of creating the blob file.
  rocksdb_rs::types::BlobFileCreationReason reason;
};
...
struct FileOperationInfo {
  ...
  FileOperationInfo(const FileOperationType _type, const std::string &_path,
                    ...
                    const FinishTimePoint &_finish_ts,
                    const rocksdb_rs::status::Status &_status,
  ...
  FileOperationInfo(const FileOperationType _type, const std::string &_path,
                    const StartTimePoint &_start_ts,
                    const FinishTimePoint &_finish_ts,
                    const rocksdb_rs::io_status::IOStatus &_status,
                    const Temperature _temperature = Temperature::kUnknown)
      : FileOperationInfo(_type, _path, _start_ts, _finish_ts, _status.status(),
                          _temperature) {}
...
struct BlobFileAdditionInfo : public BlobFileInfo {
  ...
  BlobFileAdditionInfo(const rust::String &_blob_file_path,
                       ...
                       const uint64_t _total_blob_count,
                       const uint64_t _total_blob_bytes)
      : BlobFileInfo(static_cast<std::string>(_blob_file_path),
                     _blob_file_number),
        total_blob_count(_total_blob_count),
        total_blob_bytes(_total_blob_bytes) {}
...
struct BackgroundErrorRecoveryInfo {
  ...
  BackgroundErrorRecoveryInfo()
      : old_bg_error(rocksdb_rs::status::Status_new()),
        new_bg_error(rocksdb_rs::status::Status_new()) {}
};

struct IOErrorInfo {
  IOErrorInfo(const rocksdb_rs::io_status::IOStatus &_io_status,
              FileOperationType _operation, const std::string &_file_path,
              size_t _length, uint64_t _offset)
      : io_status(_io_status.Clone()),
...
class EventListener : public Customizable {
 public:
  ...
  static rocksdb_rs::status::Status CreateFromString(
      const ConfigOptions &options, const std::string &id,
  ...
  virtual void OnErrorRecoveryCompleted(
      rocksdb_rs::status::Status old_bg_error) {}
  ...
  virtual void OnErrorRecoveryEnd(
