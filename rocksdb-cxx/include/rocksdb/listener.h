...
namespace rocksdb {
  ...
  struct BackgroundErrorRecoveryInfo {
    ...
    BackgroundErrorRecoveryInfo() : old_bg_error(rocksdb_rs::status::Status_new()),
                                    new_bg_error(rocksdb_rs::status::Status_new()) {
    }
  };

  struct IOErrorInfo {
    IOErrorInfo(const rocksdb_rs::io_status::IOStatus &_io_status, FileOperationType _operation,
