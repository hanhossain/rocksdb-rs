...
rocksdb_rs::status::Status OpenDb(const Options& options, const std::string& dbname,
              std::unique_ptr<DB>* db) {
  db->reset();
  DB* tmpdb;
  rocksdb_rs::status::Status s = DB::Open(options, dbname, &tmpdb);
  if (s.ok()) {
    db->reset(tmpdb);
  }
  return s;
...
rocksdb_rs::status::Status CompactToLevel(const Options& options, const std::string& dbname,
  ...
  rocksdb_rs::status::Status s = OpenDb(no_compact_opts, dbname, &db);
  if (!s.ok()) {
    return s;
  }
...
rocksdb_rs::status::Status MigrateToUniversal(std::string dbname, const Options& old_opts,
  ...
  if (old_opts.num_levels <= new_opts.num_levels ||
    ...
    bool need_compact = false;
    {
      ...
      rocksdb_rs::status::Status s = OpenDb(opts, dbname, &db);
      if (!s.ok()) {
        return s;
      }
...
rocksdb_rs::status::Status MigrateToLevelBase(std::string dbname, const Options& old_opts,
                          const Options& new_opts) {
